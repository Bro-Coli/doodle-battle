...
export class EntitySchema extends Schema {
  ...
  @type('string') movementStyle: string = '';
  @type('string') habitat: string = 'land';
  // Speeds: 0 sentinel = "undefined" (creature cannot survive that map).
  @type('number') landSpeed: number = 0;
  @type('number') waterSpeed: number = 0;
  @type('number') airSpeed: number = 0;
  @type('number') agility: number = 5;
  @type('number') energy: number = 5;
...
export class GameRoom extends Room<{ state: GameState }> {
  ...
  _advancePhase(): void {
    ...
    if (this.state.currentPhase === 'draw') {
      ...
      for (const [sessionId, { profile, teamId, imageDataUrl }] of this._pendingProfiles) {
        ...
        schema.movementStyle = init.movementStyle;
        schema.habitat = profile.habitat;
        schema.landSpeed = profile.landSpeed ?? 0;
        schema.waterSpeed = profile.waterSpeed ?? 0;
        schema.airSpeed = profile.airSpeed ?? 0;
        schema.agility = profile.agility;
        schema.energy = profile.energy;
  ...
  _handleSpawnEntity(_client: { sessionId: string }, msg: unknown): void {
    ...
    schema.movementStyle = profile.movementStyle;
    schema.habitat = profile.habitat;
    schema.landSpeed = profile.landSpeed ?? 0;
    schema.waterSpeed = profile.waterSpeed ?? 0;
    schema.airSpeed = profile.airSpeed ?? 0;
